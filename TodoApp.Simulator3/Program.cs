...
namespace TodoApp.ClientSimulator
{
    public class ClientSimulator
    {
        ...
        private readonly bool m_cleanupCreatedTasks;
        private readonly List<int> m_createdTaskIds = new();

        private static readonly ConcurrentBag<ClientStats> s_stats = new();

        public ClientSimulator(string baseUrl, string userName, int totalRounds = 3, int additions = 2, int completionToggles = 2, bool cleanupCreatedTasks = false)
        {
            ...
            m_cleanupCreatedTasks = cleanupCreatedTasks;
        }
        ...
        public async Task RunAsync()
        {
            ...
            if (m_cleanupCreatedTasks)
            {
                foreach (var taskId in m_createdTaskIds)
                {
                    var resp = await m_httpClient.DeleteAsync($"api/tasks/{taskId}");
        ...
        private async Task AddNewTaskAsync(ClientStats stats)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (createdTask != null) m_createdTaskIds.Add(createdTask.Id);
                stats.Added++;
            }
            else
            {
        ...
        public static async Task Main(string[] args)
        {
            ...
            const int additionsPerClient = 3;
            const int completionToggles = 2;
            const bool cleanupTasks = true;

            var tasks = new List<Task>();
            for (int i = 0; i < clientCount; i++)
            {
                string user = $"User{i}";
                var simulator = new ClientSimulator(baseUrl, user, totalRounds, additionsPerClient, completionToggles, cleanupTasks);
            ...
            foreach (var stat in s_stats)
            {
                ...
                totalLockCount += stat.Updated;
                totalUpdateCount += stat.Updated;
                totalCompletionCount += stat.Completed;
            }
            ...
            Console.WriteLine($"Avg Lock Time: {totalStats.TotalLockTime / Math.Max(totalLockCount, 1)} ms");
            Console.WriteLine($"Avg Update Time: {totalStats.TotalUpdateTime / Math.Max(totalUpdateCount, 1)} ms");
            Console.WriteLine($"Avg Completion Time: {totalStats.TotalCompleteTime / Math.Max(totalCompletionCount, 1)} ms");
